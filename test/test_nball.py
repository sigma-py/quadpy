...
@pytest.mark.parametrize(
    ...
    + [quadpy.nball.Stroud(dim, index) for dim in range(2, 9)
        for index in [
        'Sn 2-1',
        'Sn 3-1', 'Sn 3-2',
        'Sn 5-2', 'Sn 5-3', 'Sn 5-4', 'Sn 5-5', 'Sn 5-6',
        ]]
    + [quadpy.nball.Stroud(dim, index) for dim in range(4, 8)
        for index in [
        'Sn 5-1a', 'Sn 5-1b',
        ]]
    + [quadpy.nball.Stroud(dim, index) for dim in range(3, 8)
        for index in [
        'Sn 7-1a'
        ]]
    + [quadpy.nball.Stroud(dim, index) for dim in range(3, 7)
        for index in [
        'Sn 7-1b', 'Sn 7-2', 'Sn 7-3a', 'Sn 7-3b',
        'Sn 9-1a',
        ]]
    + [quadpy.nball.Stroud(dim, index) for dim in range(4, 7)
        for index in [
        'Sn 9-1b',
        ]]
    + [quadpy.nball.Stroud(dim, index) for dim in [3, 4]
        for index in [
        'Sn 11-1a',
        ]]
    + [quadpy.nball.Stroud(dim, index) for dim in [4, 5]
        for index in [
        'Sn 11-1b',
        ]]
    )
