...
class Dunavant0(object):
    def __init__(self, symbolic=False):
        self.degree = 1
        self.weights, self.points = zero(4)
        return


class Dunavant1(object):
    def __init__(self, symbolic=False):
        frac = sympy.Rational if symbolic else lambda x, y: x / y
        sqrt = sympy.sqrt if symbolic else numpy.sqrt
        self.degree = 3
        self.weights, self.points = symm_s([1, sqrt(frac(1, 3))])
        return


class Dunavant2(object):
    def __init__(self, symbolic=False):
        frac = sympy.Rational if symbolic else lambda x, y: x / y
        sqrt = sympy.sqrt if symbolic else numpy.sqrt
        self.degree = 5
