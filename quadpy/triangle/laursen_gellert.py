...
Abstract:
For a wide class of finite element matrices integrated numerically rather than exactly,
a definable number of sampling points is found to be sufficient for keeping their
theoretical properties unchanged. A systematic criterion limiting the number of possible
point configurations for numerical quadrature formulas on triangles is established. Some
new high order formulas are presented. Tables containing optimal formulas with respect
to minimum number of sampling points and required degrees of accuracy are given. They
are arranged so as to assist with selection of suitable quadrature formulas for finite
element computer programming.
"""
from __future__ import division

import sympy

from .helpers import s3, TriangleScheme, s2, concat, s1


def laursen_gellert_1(symbolic=False):
    weights, bary = s3(1)
    return TriangleScheme("Laursen-Gellert 1", 1, weights, bary)


def laursen_gellert_2a(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y
    weights, bary = s2([frac(1, 3), frac(1, 6)])
    return TriangleScheme("Laursen-Gellert 2a", 2, weights, bary)


def laursen_gellert_2b(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y
    weights, bary = s2([frac(1, 3), frac(1, 2)])
    return TriangleScheme("Laursen-Gellert 2b", 2, weights, bary)


def laursen_gellert_3(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y
    weights, bary = concat(s3(-frac(9, 16)), s2([frac(25, 48), frac(1, 5)]))
    return TriangleScheme("Laursen-Gellert 3", 3, weights, bary)


def laursen_gellert_4(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y
    weights, bary = s1([frac(1, 6), 0.659027622374092, 0.231933368553031])
    return TriangleScheme("Laursen-Gellert 4", 3, weights, bary)


def laursen_gellert_5():
    weights, bary = s2(
        [0.109951743655322, 0.091576213509771], [0.223381589678011, 0.445948490915965]
    )
    return TriangleScheme("Laursen-Gellert 5", 4, weights, bary)


def laursen_gellert_6(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y
    weights, bary = concat(
        s3(frac(3, 8)), s1([frac(5, 48), 0.736712498968435, 0.237932366472434])
    )
    return TriangleScheme("Laursen-Gellert 6", 4, weights, bary)


def laursen_gellert_7(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y
    weights, bary = concat(
        s3(frac(9, 40)),
        s2(
    ...
    return TriangleScheme("Laursen-Gellert 7", 5, weights, bary)


def laursen_gellert_8():
    weights, bary = concat(
        s2([0.205950504760887, 0.437525248383384]),
        s1([0.063691414286223, 0.797112651860071, 0.165409927389841]),
    )
    return TriangleScheme("Laursen-Gellert 8", 5, weights, bary)


def laursen_gellert_9():
    weights, bary = concat(
        s2(
    ...
    return TriangleScheme("Laursen-Gellert 9", 6, weights, bary)
...
def laursen_gellert_10():
    ...
    return TriangleScheme("Laursen-Gellert 10", 7, weights, bary)
...
def laursen_gellert_11():
    ...
    return TriangleScheme("Laursen-Gellert 11", 7, weights, bary)
...
def laursen_gellert_12():
    ...
    return TriangleScheme("Laursen-Gellert 12", 8, weights, bary)
...
def laursen_gellert_13():
    ...
    return TriangleScheme("Laursen-Gellert 13", 9, weights, bary)
...
def laursen_gellert_14():
    ...
    return TriangleScheme("Laursen-Gellert 14", 9, weights, bary)
...
def laursen_gellert_15a():
    ...
    return TriangleScheme("Laursen-Gellert 15a", 10, weights, bary)
...
def laursen_gellert_15b():
    ...
    return TriangleScheme("Laursen-Gellert 15b", 10, weights, bary)


LaursenGellert = {
    "1": laursen_gellert_1,
    "2a": laursen_gellert_2a,
    "2b": laursen_gellert_2b,
    "3": laursen_gellert_3,
    "4": laursen_gellert_4,
    "5": laursen_gellert_5,
    "6": laursen_gellert_6,
    ...
    "8": laursen_gellert_8,
    "9": laursen_gellert_9,
    "10": laursen_gellert_10,
    "11": laursen_gellert_11,
    "12": laursen_gellert_12,
    "13": laursen_gellert_13,
    "14": laursen_gellert_14,
    "15a": laursen_gellert_15a,
    "15b": laursen_gellert_15b,
}
