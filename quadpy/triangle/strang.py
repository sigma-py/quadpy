# -*- coding: utf-8 -*-
#
"""
See
https://people.sc.fsu.edu/~jburkardt/datasets/quadrature_rules_tri/quadrature_rules_tri.html
and

Gilbert Strang, George Fix,
An Analysis of the Finite Element Method,
Cambridge, 1973,
ISBN: 096140888X,
LC: TA335.S77,
<https://bookstore.siam.org/wc08/>.

The same schemes are published as

G.R. Cowper,
Gaussian quadrature formulas for triangles,
Numerical Methods in Engineering,
Volume 7, Issue 3, 1973, Pages 405–408.
<https://doi.org/10.1002/nme.1620070316>.
"""
from __future__ import division

import sympy

from .helpers import TriangleScheme, concat, s1, s2, s3


def strang_1(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y

    weights, bary = s2([frac(1, 3), frac(1, 6)])
    return TriangleScheme("Strang 1", 2, weights, bary)


def strang_2(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y

    weights, bary = s2([frac(1, 3), frac(1, 2)])
    return TriangleScheme("Strang 2", 2, weights, bary)


def strang_3(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y

    weights, bary = concat(s3(-frac(9, 16)), s2([frac(25, 48), frac(1, 5)]))
    return TriangleScheme("Strang 3", 3, weights, bary)


def strang_4(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y

    weights, bary = s1([frac(1, 6), 0.659027622374092, 0.231933368553031])
    return TriangleScheme("Strang 4", 3, weights, bary)


def strang_5():
    weights, bary = s2(
        [0.109951743655322, 0.091576213509771], [0.223381589678011, 0.445948490915965]
    )
    return TriangleScheme("Strang 5", 4, weights, bary)


def strang_6(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y

    weights, bary = concat(
        s3(frac(3, 8)), s1([frac(5, 48), 0.736712498968435, 0.237932366472434])
    )
    return TriangleScheme("Strang 6", 4, weights, bary)


def strang_7(symbolic=False):
    frac = sympy.Rational if symbolic else lambda x, y: x / y

    weights, bary = concat(
        s3(frac(9, 40)),
        s2(
            [0.12593918054482717, 0.10128650732345633],
            [0.13239415278850616, 0.47014206410511505],
        ),
    )
    return TriangleScheme("Strang 7", 5, weights, bary)


def strang_8(symbolic=False):
    weights, bary = concat(
        s2([0.205950504760887, 0.437525248383384]),
        s1([0.063691414286223, 0.797112651860071, 0.165409927389841]),
    )
    return TriangleScheme("Strang 8", 5, weights, bary)


def strang_9(symbolic=False):
    weights, bary = concat(
        s2(
            [0.050844906370207, 0.063089014491502],
            [0.116786275726379, 0.249286745170910],
        ),
        s1([0.082851075618374, 0.636502499121399, 0.310352451033785]),
    )
    return TriangleScheme("Strang 9", 6, weights, bary)


def strang_10():
    weights, bary = concat(
        s3(-0.149570044467670),
        s2(
            [+0.175615257433204, 0.260345966079038],
            [+0.053347235608839, 0.065130102902216],
        ),
        s1([+0.077113760890257, 0.638444188569809, 0.312865496004875]),
    )
    return TriangleScheme("Strang 10", 7, weights, bary)


Strang = {
    1: strang_1,
    2: strang_2,
    3: strang_3,
    4: strang_4,
    5: strang_5,
    6: strang_6,
    7: strang_7,
    8: strang_8,
    9: strang_9,
    10: strang_10,
}
